---
TOCTitle: 'MS11-059'
Title: 'Biuletyn zabezpieczeń firmy Microsoft MS11-059 - Ważna'
ms:assetid: 'ms11-059'
ms:contentKeyID: 61232958
ms:mtpsurl: 'https://technet.microsoft.com/pl-PL/library/ms11-059(v=Security.10)'
---

Security Bulletin

Biuletyn zabezpieczeń firmy Microsoft MS11-059 - Ważna
======================================================

Luka w zabezpieczeniach składników Data Access Components umożliwia zdalne wykonanie kodu (2560656)
---------------------------------------------------------------------------------------------------
...
Aktualizacja usuwa lukę, poprawiając sposób ładowania bibliotek zewnętrznych przez składnik Windows Data Access Tracing. Więcej informacji na temat luki można znaleźć w podsekcji „Często zadawane pytania” w części poświęconej tej luce w następnej sekcji, **Informacje o luce w zabezpieczeniach**.
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|----------------------------|----------------------------------------------------|
| [Windows 7 dla systemów 32-bitowych i Windows 7 z dodatkiem Service Pack 1 dla systemów 32-bitowych](http://www.microsoft.com/downloads/details.aspx?familyid=6bc168d9-6664-41fb-914f-dbd7514a4b0e)                                                                   | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows 7 dla systemów opartych na procesorach x64 i Windows 7 z dodatkiem Service Pack 1 dla systemów opartych na procesorach x64](http://www.microsoft.com/downloads/details.aspx?familyid=aafeff2d-db9a-4b3b-b620-be4ec5f5bdcc)                                   | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows Server 2008 R2 dla systemów opartych na procesorach x64 i Windows Server 2008 R2 z dodatkiem Service Pack 1 dla systemów opartych na procesorach x64](http://www.microsoft.com/downloads/details.aspx?familyid=c29deec3-4672-4658-a550-dce244434cd4)\*       | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows Server 2008 R2 dla systemów opartych na procesorach Itanium i Windows Server 2008 R2 z dodatkiem Service Pack 1 dla systemów opartych na procesorach Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=1d6b8036-d38f-408a-a36c-027553faefc1) | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
...
Tak. Luka w zabezpieczeniach składników Data Access Components związana z niebezpiecznym ładowaniem biblioteki (CVE-2011-1975) usuwana przez niniejszą aktualizację jest powiązana z klasą luk opisanych w [Poradniku zabezpieczeń firmy Microsoft 2269637](http://technet.microsoft.com/security/advisory/2269637), które dotyczą sposobu ładowania bibliotek zewnętrznych przez aplikacje. Niniejsza aktualizacja usuwa konkretne wystąpienie luki tego rodzaju.
...
<th style="border:1px solid black;" >Luka w zabezpieczeniach składników Data Access Components związana z niebezpiecznym ładowaniem biblioteki — CVE-2011-1975</th>
...
Luka w zabezpieczeniach składników Data Access Components związana z niebezpiecznym ładowaniem biblioteki — CVE-2011-1975  
-------------------------------------------------------------------------------------------------------------------------
  
<span></span>
Sposób, w jaki składnik Windows Data Access Tracing obsługuje ładowanie plików DLL, powoduje powstanie luki w zabezpieczeniach umożliwiającej zdalne wykonanie kodu. Osoba atakująca, której uda się wykorzystać tę lukę, może uzyskać pełną kontrolę nad systemem, którego dotyczy luka. Osoba taka mogłaby wówczas instalować programy, przeglądać, zmieniać i usuwać dane oraz tworzyć nowe konta z pełnymi uprawnieniami. Użytkownicy, których konta zostały skonfigurowane w taki sposób, że mają niewielkie uprawnienia w systemie, ponoszą mniejsze ryzyko niż ci, którzy pracują z uprawnieniami administracyjnymi.
  
Aby wyświetlić tę lukę w zabezpieczeniach jako standardową pozycję na liście znanych luk i zagrożeń, zobacz [CVE-2011-1975](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1975).
  
#### Czynniki ograniczające zagrożenie wynikające z luki w zabezpieczeniach składników Data Access Components związanej z niebezpiecznym ładowaniem biblioteki — CVE-2011-1975
  
Ograniczenie sprowadza się do odpowiedniego ustawienia, typowej konfiguracji lub najważniejszej wskazówki ogólnej, które istnieją w stanie domyślnym i mogą zmniejszyć zagrożenie wykorzystaniem omawianej luki w zabezpieczeniach. W tej sytuacji pomocne mogą być następujące czynniki ograniczające zagrożenie:
  
-   Aby atak mógł się powieść, użytkownik musi odwiedzić niezaufaną zdalną lokalizację systemu plików lub udział WebDAV i otworzyć plik programu Excel (taki jak .xlsx).  
...
#### Obejścia problemu spowodowanego przez lukę w zabezpieczeniach składników Data Access Components związaną z niebezpiecznym ładowaniem biblioteki — CVE-2011-1975
...
#### Często zadawane pytania dotyczące luki w zabezpieczeniach składników Data Access Components związanej z niebezpiecznym ładowaniem biblioteki — CVE-2011-1975
...
Luka powstaje, gdy składnik Windows Data Access Tracing nieprawidłowo ogranicza ścieżkę używaną do ładowania bibliotek zewnętrznych.
  
**Co to są składniki Windows Data Access Components (Windows DAC)?**    
Składniki Windows Data Access Components (Windows DAC) 6.0 to zestaw technologii, w które wyposażone są systemy Windows Vista, Windows 7, Windows Server 2008 oraz Windows Server 2008 R2. Ich zadaniem jest zapewnianie dostępu do informacji w całej organizacji. Technologie te obejmują m.in. rozwiązania Microsoft ActiveX Data Objects (ADO), OLE DB oraz Microsoft Open Database Connectivity (ODBC). Aplikacje klienckie/serwerowe oparte na danych, które są wdrażane za pośrednictwem sieci Web lub LAN mogą wykorzystać te składniki w celu łatwej integracji informacji relacyjnych (SQL) i nierelacyjnych pochodzących z wielu źródeł. Więcej informacji można znaleźć w artykule MSDN [Windows Data Access Components SDK](http://msdn.microsoft.com/en-us/library/aa968814(v=vs.85).aspx).
...
Osoba atakująca może przekonać użytkownika do otwarcia prawidłowego pliku programu Excel (na przykład .xlsx), który znajduje się w tym samym katalogu sieciowym, co specjalnie spreparowany plik biblioteki dołączanej dynamicznie (biblioteki DLL). Następnie podczas otwierania prawidłowego pliku pakiet Microsoft Office może podjąć próbę załadowania pliku DLL i wykonania dowolnego zawartego w nim kodu.
  
Osoba atakująca za pośrednictwem poczty elektronicznej może wykorzystać omawianą lukę, wysyłając do użytkownika w załączniku prawidłowy plik programu Excel (na przykład .xslx) i przekonując go do umieszczenia tego załącznika w katalogu, który zawiera specjalnie spreparowany plik DLL i do otwarcia go. Następnie podczas otwierania prawidłowego pliku pakiet Microsoft Office może podjąć próbę załadowania pliku DLL i wykonania dowolnego zawartego w nim kodu.
...
Choć znane są kierunki ataku wykorzystujące oprogramowanie Microsoft Office, takie jak np. Microsoft Excel, niebezpieczny kod znajduje się we współdzielonym składniku systemu Microsoft Windows.
...
Aktualizacja usuwa tę lukę, poprawiając sposób ładowania bibliotek zewnętrznych przez składnik Windows Data Access Tracing.
...
<td style="border:1px solid black;">Zobacz <a href="http://support.microsoft.com/kb/2560656">artykuł 2560656 bazy wiedzy Microsoft Knowledge Base</a></td>
</tr>
<tr class="odd">
