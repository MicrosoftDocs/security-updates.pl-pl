...
ms:contentKeyID: 21798548
ms:mtpsurl: 'https://technet.microsoft.com/pl-pl/library/Dd939812(v=WS.10)'
---

Choose the Database Used for WSUS 3.0
=====================================

You do not need to be a database administrator or purchase database software to use WSUS. WSUS 3.0 will install Wewnętrzna baza danych systemu Windows if you choose to install a minimal version of SQL Server. This version of SQL Server is designed to require very little management by the WSUS administrator. If you already have Windows Internal Database installed, WSUS will use it. If you want more control over the database, you can also use the full version of SQL Server with WSUS.
...
<thead>
<tr class="header">
<th style="border:1px solid black;" ><img src="images/Dd939812.Important(WS.10).gif" />Ważne</th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
-   Wewnętrzna baza danych systemu Windows ships with WSUS 3.0. This version of SQL Server does not have a user interface or tools. Administrators are meant to interact with these products through WSUS.
...
You cannot use SQL authentication with WSUS, which supports only Windows authentication. If you choose Wewnętrzna baza danych systemu Windows for the WSUS database, WSUS Setup creates an instance of SQL Server named *server*\\MICROSOFT\#\#SSEE, where *server* is the name of the computer. With either database option, WSUS Setup creates a database named SUSDB. The name of this database is not configurable.
