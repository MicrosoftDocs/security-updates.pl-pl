---
TOCTitle: 'MS10-095'
Title: 'Biuletyn zabezpieczeń firmy Microsoft MS10-095 - Ważna'
ms:assetid: 'ms10-095'
ms:contentKeyID: 61232888
ms:mtpsurl: 'https://technet.microsoft.com/pl-PL/library/ms10-095(v=Security.10)'
---

Security Bulletin

Biuletyn zabezpieczeń firmy Microsoft MS10-095 - Ważna
======================================================

Luka w zabezpieczeniach systemu Microsoft Windows może pozwolić na zdalne wykonanie kodu (2385678)
--------------------------------------------------------------------------------------------------
...
Niniejsza aktualizacja zabezpieczeń ma wskaźnik ważności „ważny” dla wszystkich obsługiwanych wersji systemu Windows 7 i Windows Server 2008 R2. Więcej informacji można znaleźć w podsekcji **Programy, których dotyczy problem i programy, których nie dotyczy problem** niniejszej sekcji.

Aktualizacja usuwa lukę, poprawiając sposób ładowania bibliotek zewnętrznych przez usługę BranchCache systemu Windows. Więcej informacji na temat luki można znaleźć w podsekcji „Często zadawane pytania” w części poświęconej tej luce w następnej sekcji, **Informacje o luce w zabezpieczeniach**.
...
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|----------------------------|----------------------------------------------------|
| [Windows 7 dla systemów 32-bitowych](http://www.microsoft.com/downloads/details.aspx?familyid=75591d37-2cb8-4cdf-acbb-89cd0d1a9290)                                  | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows 7 dla systemów z procesorem x64](http://www.microsoft.com/downloads/details.aspx?familyid=3a0c4dd0-98b4-4e7a-99ed-22b9d9f76cd1)                             | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows Server 2008 R2 dla systemów opartych na procesorach x64](http://www.microsoft.com/downloads/details.aspx?familyid=f58a765f-cea9-456d-b0ab-bfc70b109cbf)\*   | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
| [Windows Server 2008 R2 dla systemów opartych na procesorach Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=13a9f838-ac07-43dc-9aee-a77207998e1e) | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
...
<th style="border:1px solid black;" >Luka w zabezpieczeniach związana z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache — CVE-2010-3966</th>
...
Luka w zabezpieczeniach związana z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache — CVE-2010-3966  
---------------------------------------------------------------------------------------------------------------
  
<span></span>
W systemie Microsoft Windows istnieje luka w zabezpieczeniach umożliwiająca zdalne wykonanie kodu, związana ze sposobem otwierania określonych plików na platformach nieobsługujących funkcji BranchCache. Osoba atakująca, której uda się wykorzystać tę lukę, może uzyskać pełną kontrolę nad systemem, którego dotyczy luka. Osoba taka mogłaby wówczas instalować programy, przeglądać, zmieniać i usuwać dane oraz tworzyć nowe konta z pełnymi uprawnieniami. Użytkownicy, których konta zostały skonfigurowane w taki sposób, że mają niewielkie uprawnienia w systemie, ponoszą mniejsze ryzyko niż ci, którzy pracują z uprawnieniami administracyjnymi.
  
Aby wyświetlić tę lukę w zabezpieczeniach jako standardową pozycję na liście znanych luk i zagrożeń, zobacz [CVE-2010-3966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3966).
  
#### Czynniki ograniczające zagrożenie wynikające z luki w zabezpieczeniach związanej z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache — CVE-2010-3966
...
#### Obejścia problemu spowodowanego przez lukę w zabezpieczeniach związaną z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache — CVE-2010-3966
...
#### Często zadawane pytania dotyczące luki w zabezpieczeniach związanej z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache — CVE-2010-3966
...
Luka w zabezpieczeniach powstaje w chwili, gdy system Windows podejmuje próbę sprawdzenia, czy określona biblioteka DLL używana przez usługę BranchCache istnieje w danym systemie. W przypadku, gdy domyślnej biblioteki DLL nie ma w systemie, będzie ona w pierwszej kolejności wyszukiwana w bezpiecznych, lokalnych folderach systemu Windows. Jeśli jednak nie zostanie odnaleziona, aplikacja podejmie próbę jej wyszukania w lokalizacji, z której został otwarty plik, i która może być niebezpieczna jak np. udział sieciowy. Specjalnie spreparowany plik umieszczony w alternatywnej lokalizacji może spowodować uruchomienie w systemie Windows wybranego przez osobę atakującą kodu w kontekście zalogowanego użytkownika.
  
Ta konkretna biblioteka ładowana przez usługę BranchCache systemu Windows znajduje się jedynie w wersjach systemu Windows z obsługą funkcji BranchCache. Wymienione platformy to systemy Windows 7 Enterprise i Ultimate oraz Windows Server 2008 R2. Inne wersje systemu Windows 7 nie zawierają tego pliku binarnego w odpowiedniej lokalizacji systemowej i domyślnie ten problem ich dotyczy. Wszystkie wersje systemów Windows 7 oraz Windows Server 2008 R2 mogą być narażone na wykorzystanie omawianej luki, jeśli biblioteka DLL BranchCache nie znajduje się w domyślnej lokalizacji.
  
**Co to jest Windows BranchCache?**    
BranchCache to technologia optymalizacji wykorzystania pasma w sieciach WAN zawarta w niektórych wersjach systemów operacyjnych Windows Server 2008 R2 oraz Windows 7. Optymalizacja pasma w sieciach WAN polega na kopiowaniu przez usługę BranchCache treści z serwerów treści w biurze głównym firmy i przechowywania ich w pamięci podręcznej w oddziałach lokalnych, umożliwiając znajdującym się tam komputerom klienckim lokalny dostęp do tych treści zamiast zdalnego pobierania ich w sieci WAN. Więcej informacji można znaleźć w artykule [Windows Server 2008 R2 and BranchCache](http://www.microsoft.com/windowsserver2008/en/us/branch-cache.aspx).
...
Osoba atakująca może przekonać użytkownika do otwarcia prawidłowego pliku obsługiwanego przez określone aplikacje systemu Windows, na przykład pliku z rozszerzeniem .eml lub .rss programu Poczta usługi Windows Live lub pliku z rozszerzeniem .wpost programu Microsoft Live Writer, znajdującego się w tym samym katalogu sieciowym co specjalnie spreparowany plik biblioteki dołączanej dynamicznie (DLL). Następnie, podczas otwierania prawidłowego pliku usługa BranchCache podjęłaby próbę załadowania pliku DLL i wykonania dowolnego zawartego w nim kodu.
  
Osoba atakująca za pośrednictwem poczty e-mail może wykorzystać tę lukę, wysyłając do użytkownika wiadomość z formalnie poprawnym załącznikiem typu .eml, .rss lub .wpost i przekonując go do umieszczenia załączonego pliku w katalogu zawierającym specjalnie spreparowany plik DLL oraz otwarcia poprawnego pliku. Następnie, podczas otwierania poprawnego pliku program Windows Live Mail lub Microsoft Live Writer może podjąć próbę załadowania pliku DLL i uruchomienia zawartego w nim kodu.
  
Osoba atakująca za pośrednictwem sieci może umieścić prawidłowy plik wraz ze specjalnie spreparowanym plikiem DLL w lokalizacji UNC lub WebDAV, a następnie przekonać użytkownika do otwarcia prawidłowego pliku.
...
Aktualizacja zabezpieczeń usuwa tę lukę, poprawiając sposób ładowania bibliotek zewnętrznych przez usługę BranchCache systemu Windows.
...
<td style="border:1px solid black;">Zobacz <a href="http://support.microsoft.com/kb/2385678">artykuł 2385678 bazy wiedzy Microsoft Knowledge Base</a></td>
</tr>
<tr class="odd">
...
-   Haifei Li z [FortiGuard Labs firmy Fortinet](http://www.fortiguard.com/), za zgłoszenie luki w zabezpieczeniach związanej z niebezpiecznym ładowaniem bibliotek przez usługę BranchCache (CVE-2010-3966)
