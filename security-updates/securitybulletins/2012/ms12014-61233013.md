---
TOCTitle: 'MS12-014'
Title: 'Biuletyn zabezpieczeń firmy Microsoft MS12-014 - Ważna'
ms:assetid: 'ms12-014'
ms:contentKeyID: 61233013
ms:mtpsurl: 'https://technet.microsoft.com/pl-PL/library/ms12-014(v=Security.10)'
---

Biuletyn zabezpieczeń firmy Microsoft MS12-014 - Ważna
======================================================

Luka w zabezpieczeniach kodera-dekodera Indeo umożliwia zdalne wykonanie kodu (2661637)
---------------------------------------------------------------------------------------

Opublikowano: 14 lutego 2012 | Zaktualizowano: 22 lutego 2012
...
Niniejsza aktualizacja zabezpieczeń ma wskaźnik ważności „ważny” dla systemu Microsoft Windows XP z dodatkiem Service Pack 3. Więcej informacji można znaleźć w podsekcji **Programy, których dotyczy problem i programy, których nie dotyczy problem** niniejszej sekcji.

Aktualizacja usuwa lukę przez skorygowanie sposobu, w jaki koder-dekoder Indeo ładuje biblioteki zewnętrzne. Więcej informacji na temat luki można znaleźć w podsekcji „Często zadawane pytania” w części poświęconej tej luce w następnej sekcji, **Informacje o luce w zabezpieczeniach**.
...
**Znane problemy.** W [artykule 2661637 bazy wiedzy Microsoft Knowledge Base](http://support.microsoft.com/kb/2661637) udokumentowano znane obecnie problemy, które mogą wystąpić przy instalacji niniejszej aktualizacji zabezpieczeń. Artykuł ten przedstawia również zalecane obejścia tych problemów.
...
|----------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|----------------------------|----------------------------------------------------|
| [Windows XP z dodatkiem Service Pack 3](http://www.microsoft.com/downloads/details.aspx?familyid=32e5c9ad-b610-4afb-b6f0-bb0b5fbdd1f6) | Zdalne wykonanie kodu              | Ważny                      | Brak                                               |
...
Tak. Luka w zabezpieczeniach kodera-dekodera Indeo związana z niebezpiecznym ładowaniem bibliotek (CVE-2010-3138) usuwana przez niniejszą aktualizację jest powiązana z klasą luk opisanych w [Poradniku zabezpieczeń firmy Microsoft 2269637](http://technet.microsoft.com/security/advisory/2269637), które dotyczą sposobu ładowania bibliotek zewnętrznych przez aplikacje. Niniejsza aktualizacja usuwa konkretne wystąpienie luki tego rodzaju.
...
<th style="border:1px solid black;" >
Luka w zabezpieczeniach kodera-dekodera Indeo związana z niebezpiecznym ładowaniem bibliotek — CVE-2010-3138
</th>
...
-   Aby atak się powiódł, użytkownik musi odwiedzić niezaufaną zdalną lokalizację systemu plików lub udział WebDAV i otworzyć plik multimedialny (na przykład .avi).

#### Obejścia problemu spowodowanego przez lukę w zabezpieczeniach kodera-dekodera Indeo związaną z niebezpiecznym ładowaniem bibliotek — CVE-2010-3138
...
#### Często zadawane pytania dotyczące luki w zabezpieczeniach kodera-dekodera Indeo związanej z niebezpiecznym ładowaniem bibliotek — CVE-2010-3138
...
Luka powstaje, ponieważ koder-dekoder Indeo nieprawidłowo ogranicza ścieżkę używaną do ładowania bibliotek zewnętrznych.

**Co to jest koder-dekoder Indeo?**  
Indeo to koder-dekoder dekompresujący pliki multimediów cyfrowych do użycia w aplikacjach takich, jak Windows Media Player. Więcej informacji na temat koderów-dekoderów znajduje się w sekcji [Kodery-dekodery: Często zadawane pytania](http://windows.microsoft.com/en-us/windows7/codecs-frequently-asked-questions).

**Co to jest plik multimedialny AVI?**  
AVI (Audio Video Interleave) to format kontenera multimedialnego opracowany przez firmę Microsoft, który korzysta z formatu RIFF (Resource Interchange File Format). Format AVI został zdefiniowany przez firmę Microsoft. Plik w formacie AVI to popularny format danych audio i wideo na komputerze.
...
Osoba atakująca może przekonać użytkownika do otwarcia prawidłowego pliku multimedialnego (na przykład .avi), który znajduje się w tym samym katalogu sieciowym, co specjalnie spreparowany plik biblioteki dołączanej dynamicznie (biblioteki DLL). Podczas otwierania prawidłowego pliku koder-dekoder Indeo podjąłby próbę załadowania pliku DLL i wykonania zawartego w nim kodu.

Osoba atakująca za pośrednictwem poczty e-mail może wykorzystać tę lukę, wysyłając do użytkownika prawidłowy plik (na przykład .avi), a następnie przekonując użytkownika do umieszczenia tego pliku w katalogu, który zawiera specjalnie spreparowany plik DLL, i do otwarcia prawidłowego pliku. Podczas otwierania prawidłowego pliku koder-dekoder Indeo podjąłby próbę załadowania pliku DLL i wykonania zawartego w nim kodu.

Osoba atakująca za pośrednictwem sieci może umieścić prawidłowy plik (na przykład .avi) oraz specjalnie spreparowany plik DLL w udziale sieciowym, ścieżce UNC lub lokalizacji WebDAV, a następnie przekonać użytkownika do otwarcia tego pliku.

**Jakie systemy są szczególnie narażone na skutki tej luki?**  
Szczególnie narażone na skutki tej luki są stacje robocze, których użytkownicy mogą być zmuszeni do otwierania plików pochodzących z niezaufanych lokalizacji sieciowych.

**W jaki sposób działa aktualizacja?**  
Aktualizacja zabezpieczeń usuwa tę lukę, poprawiając sposób ładowania bibliotek zewnętrznych przez koder-dekoder Indeo.
...
Tak. Informacje o tej luce są znane publicznie. Została ona zarejestrowana w bazie danych znanych luk i zagrożeń pod numerem [CVE-2010-3138](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3138).
...
<td style="border:1px solid black;">Użyj pozycji <strong>Dodaj lub usuń programy</strong> w Panelu sterowania lub narzędzia Spuninst.exe, które znajduje się w folderze %Windir%\$NTUninstallKB2661637$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informacje dotyczące plików</strong></td>
<td style="border:1px solid black;">Zobacz <a href="http://support.microsoft.com/kb/2661637">artykuł 2661637 bazy wiedzy Microsoft Knowledge Base</a>,</td>
</tr>
<tr class="even">
...
-   Wersja 1.1 (22 lutego 2012 r.): W części „Znane problemy” sekcji „Streszczenie” dodano łącze do artykułu 2661637 bazy wiedzy Microsoft Knowledge Base.
